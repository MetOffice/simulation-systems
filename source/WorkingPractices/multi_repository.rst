...
handling with extra care, but also that some changes that aren't expected to have
any interactions will still require extra testing to prove this.
...
sit in the middle of the circles and are independant of anything else. The UM
...
dependant on both the UM and the child repositories.
...
repository is handled can be found :ref:`here <testing>`.
    ...
    .. code-block::
        ...
        rose stem --group=developer --source=. --source=/path/to/jules/changes --source=/path/to/ukca/changes

    The source paths involved can be either to local working copies or links to the
    ...
    ``fcm-make/parameters.sh`` under each of the ``*_sources`` variables.
