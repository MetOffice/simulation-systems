...
check the output. As well as testing the UM, there are also tests for createBC,
mule, Shumlib and a variety of other utilities.
...
.. tip::
    ...
    stages of developing a change choosing a single compile and run task to test
    with may be helpful. e.g.,
...
.. note::
    ...
    :ref:`testing with LFRic Apps<lfric_apps_test>`.If you have access to LFRic,
    the :ref:`traclog` will state whether LFRic testing is required based on the
...
and that you can specify more than one at once, e.g.,
...
+--------------------+----------------------------------------------------------+
| scripts            | All of the auxiliary scripts that are designed to check  |
...
As of UM vn13.9, the UM is able to run on Monsoon3, the latest version hosted by
the Met Office EX machines. To run on here, users should follow the process laid
out in the Monsoon User Guide. This involves logging onto the ``cazccylc1``
server to launch jobs.

The UM test suite is set up to run on Monsoon with Cylc8 by running,
