...
``main`` of each project. Depending on any linked tickets and documentation
updates, there may be several commits to different trunks involved.
...
reopen the ``main`` as soon as possible. Two possible scenarios may occur:
  ...
  #. For **simple or obvious fixes**, a second commit is the preferred
     solution, allowing the change to be fixed, while remaining on ``main`` .
...
been committed to the ``main``. The most common case is when the nightly tests
